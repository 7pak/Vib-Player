...
import com.abdts.musicplayerpractice.ui.local.items.VibTopAppBar
...
fun LocalAudioScreen(
    ...
    Scaffold(
        topBar = { VibTopAppBar(audioList, navController = navController,localState, onItemClicked=onItemClicked) }
    ) { padding ->
        LazyColumn(
            ...
            itemsIndexed(audioList) { index, audio ->
                AudioItem(audio = audio, onClick = {isPlaying->
                ...
                }, isSelected = audio.id == localState.currentSelectedAudio.id, isPlaying = localState.isPlaying)
                HorizontalDivider()
            }
        }
    }
